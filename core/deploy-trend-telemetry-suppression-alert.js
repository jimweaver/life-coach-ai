...
class DeployTrendTelemetrySuppressionAlertDetector {
  constructor(options = {}) {
    this.warnCooldownShare = Number(options.warnCooldownShare ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_WARN_COOLDOWN_SHARE ?? 0.6);
    this.criticalCooldownShare = Number(options.criticalCooldownShare ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_CRITICAL_COOLDOWN_SHARE ?? 0.85);

    this.warnDuplicateWindowShare = Number(options.warnDuplicateWindowShare ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_WARN_DUPLICATE_SHARE ?? 0.6);
    this.criticalDuplicateWindowShare = Number(options.criticalDuplicateWindowShare ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_CRITICAL_DUPLICATE_SHARE ?? 0.85);

    this.warnCooldownSpike = Number(options.warnCooldownSpike ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_WARN_COOLDOWN_SPIKE ?? 1.4);
    this.criticalCooldownSpike = Number(options.criticalCooldownSpike ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_CRITICAL_COOLDOWN_SPIKE ?? 1.8);

    this.warnDuplicateWindowSpike = Number(options.warnDuplicateWindowSpike ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_WARN_DUPLICATE_SPIKE ?? 1.4);
    this.criticalDuplicateWindowSpike = Number(options.criticalDuplicateWindowSpike ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_CRITICAL_DUPLICATE_SPIKE ?? 1.8);

    this.minSuppressed = Number(options.minSuppressed ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_MIN_SUPPRESSED ?? 2);
    this.minBuckets = Number(options.minBuckets ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_MIN_BUCKETS ?? 1);
    this.baselineBuckets = Number(options.baselineBuckets ?? process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_BASELINE_BUCKETS ?? 3);
  }
    ...
    return (Array.isArray(buckets) ? buckets : [])
      .map((bucket) => {
        ...
        return {
          ...
          detected: Math.max(0, toNum(bucket.detected)),
          route_candidate: Math.max(0, toNum(bucket.route_candidate)),
          route_attempted: Math.max(0, toNum(bucket.route_attempted)),
          route_delivered: Math.max(0, toNum(bucket.route_delivered)),
          route_failed: Math.max(0, toNum(bucket.route_failed)),
...
module.exports = DeployTrendTelemetrySuppressionAlertDetector;
