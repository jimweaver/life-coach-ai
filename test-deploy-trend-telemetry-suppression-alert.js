...
async function run() {
  ...
  process.env.DEPLOY_TREND_SUPPRESSION_STATE_KEY = `lifecoach:test:deploy-trend-telemetry-suppression-alert:anomaly:${Date.now()}:${process.pid}`;
  ...
  process.env.DEPLOY_TREND_TELEMETRY_ALERT_STATE_KEY = `lifecoach:test:deploy-trend-telemetry-suppression-alert:telemetry:${Date.now()}:${process.pid}`;
  ...
  const runId = uuidv4();
  const source = 'test-deploy-trend-telemetry-suppression-alert';
  const sink = new DeployEventSink({ runId, source });
  const db = new DatabaseStorageManager();

  try {
    const marker = { test_suite: 'deploy_trend_telemetry_suppression_alert' };
    const now = Date.now();
    ...
    try {
      ...
      const res = await fetch(`${base}/jobs/deploy-events/anomalies/telemetry/alerts/suppression/anomalies?runId=${runId}&source=${source}&sinceMinutes=360&bucketMinutes=60&route=true&emitAudit=true`);
      assert(res.status === 200, `suppression anomalies endpoint expected 200, got ${res.status}`);
      const body = await res.json();

      assert(body.ok === true, 'suppression anomalies payload ok should be true');
      assert(body.alert?.alert_detected === true, 'suppression anomalies should detect alert');
      assert(body.route?.candidate === true, 'suppression anomalies route candidate should be true');
      assert(body.routed?.attempted === true, 'suppression anomalies route should be attempted');
      ...
      const bad = await fetch(`${base}/jobs/deploy-events/anomalies/telemetry/alerts/suppression/anomalies?bucketMinutes=0`);
      assert(bad.status === 400, `invalid bucketMinutes expected 400, got ${bad.status}`);

      const logs = await db.getAgentLogs('deploy-trend', 120);
      const auditHit = logs.find((x) => x.action === 'deploy_trend_telemetry_suppression_alert_detected');
      assert(!!auditHit, 'expected deploy_trend_telemetry_suppression_alert_detected audit log');

      console.log('✅ deploy trend telemetry suppression alert test passed');
    } finally {
      await shutdown();
    }

    await db.ensureDeployRunEventsTable();
    await db.postgres.query(`DELETE FROM deploy_run_events WHERE payload->>'test_suite' = 'deploy_trend_telemetry_suppression_alert'`);
...
run().catch((err) => {
  console.error('❌ deploy trend telemetry suppression alert test failed:', err.message);
  process.exit(1);
});
