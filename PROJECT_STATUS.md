...
54. **Deploy anomaly telemetry trend rollups added (time-bucketed suppression/routing visibility)**
   - Added DB helper: `getDeployTrendAnomalyTelemetryTrend(...)`
   - Added endpoint: `GET /jobs/deploy-events/anomalies/telemetry/trend`
   - Supports filters: `runId`, `source`, `sinceMinutes`, `bucketMinutes`, `limit`, `bucketLimit`
   - Returns bucketed counts and rates: `detected`, `suppressed`, `route_attempted`, `route_delivered`, `route_failed`
   - `GET /jobs/deploy-events/dashboard` now supports `includeTelemetryTrend=true|false` and returns `anomaly_telemetry_trend`
   - Added coverage: `test-deploy-trend-telemetry-trend.js`
...
57. **Deploy telemetry-alert suppression observability endpoint added**
   - Added endpoint: `GET /jobs/deploy-events/anomalies/telemetry/alerts/suppression`
   - Returns route candidacy + suppression snapshot (`remaining_ms`, duplicate/cooldown reason, state source)
   - Supports filters: `runId`, `source`, `sinceMinutes`, `bucketMinutes`, `limit`, `bucketLimit`
   - Supports route simulation controls: `route`, `routeMinLevel`
   - Added coverage: `test-deploy-trend-telemetry-alert-suppression-observability.js`
...
61. **Telemetry suppression-alert routing suppression observability endpoint added**
   - Added endpoint: `GET /jobs/deploy-events/anomalies/telemetry/alerts/suppression/anomalies/suppression`
   - Returns route candidacy + suppression snapshot (`remaining_ms`, duplicate/cooldown reason, state source)
   - Supports filters: `runId`, `source`, `sinceMinutes`, `bucketMinutes`, `limit`, `bucketLimit`
   - Supports route simulation controls: `route`, `routeMinLevel`
   - Added coverage: `test-deploy-trend-telemetry-suppression-alert-suppression-observability.js`
   - Added npm script: `test:deploy-telemetry-suppression-alert-suppression-observability`
   - Deployment ops + readiness docs updated with suppression observability gate

62. **Telemetry suppression-alert suppression trend rollups added (route-state transitions)**
   - Added DB helper: `getDeployTrendTelemetrySuppressionAlertRouteSuppressionTrend(...)`
   - Added endpoint: `GET /jobs/deploy-events/anomalies/telemetry/alerts/suppression/anomalies/suppression/trend`
   - Supports filters: `runId`, `source`, `sinceMinutes`, `bucketMinutes`, `limit`, `bucketLimit`
   - Returns time-bucketed transition rollups with reason breakdown:
     - `route_suppressed_cooldown`
     - `route_suppressed_duplicate_window`
     - `route_suppressed_other`
   - Includes totals + rates (`suppression_rate`, `route_attempt_rate`, `route_failure_rate`)
   - Added coverage: `test-deploy-trend-telemetry-suppression-alert-suppression-trend.js`
...
npm run test:deploy-telemetry-suppression-alert-suppression-trend
