...
async function run() {
  ...
  process.env.DEPLOY_TREND_SUPPRESSION_STATE_KEY = `lifecoach:test:deploy-trend-telemetry-suppress-alert-suppress-trend:anomaly:${Date.now()}:${process.pid}`;
  ...
  process.env.DEPLOY_TREND_TELEMETRY_ALERT_STATE_KEY = `lifecoach:test:deploy-trend-telemetry-suppress-alert-suppress-trend:telemetry:${Date.now()}:${process.pid}`;
  ...
  process.env.DEPLOY_TREND_TELEMETRY_SUPPRESSION_ALERT_STATE_KEY = `lifecoach:test:deploy-trend-telemetry-suppress-alert-suppress-trend:route:${Date.now()}:${process.pid}`;
  ...
  const runId = uuidv4();
  const source = 'test-deploy-trend-telemetry-suppress-alert-suppress-trend';
  const sink = new DeployEventSink({ runId, source });
  const db = new DatabaseStorageManager();

  try {
    const marker = { test_suite: 'deploy_trend_telemetry_suppression_alert_suppression_trend' };
    const now = Date.now();
    ...
    try {
      ...
      assert(s1.status === 200, `suppression-anomalies call #1 expected 200, got ${s1.status}`);
      ...
      assert(s2.status === 200, `suppression-anomalies call #2 expected 200, got ${s2.status}`);

      const trendRes = await fetch(`${base}/jobs/deploy-events/anomalies/telemetry/alerts/suppression/anomalies/suppression/trend?runId=${runId}&source=${source}&sinceMinutes=360&bucketMinutes=60`);
      ...
      assert(trendBody.trend?.totals?.route_candidate >= 1, 'totals.route_candidate should be >=1');
      assert(trendBody.trend?.totals?.route_attempted >= 1, 'totals.route_attempted should be >=1');
      assert(trendBody.trend?.totals?.route_suppressed_total >= 1, 'totals.route_suppressed_total should be >=1');
      ...
      const bad = await fetch(`${base}/jobs/deploy-events/anomalies/telemetry/alerts/suppression/anomalies/suppression/trend?bucketLimit=0`);
      assert(bad.status === 400, `invalid bucketLimit should return 400, got ${bad.status}`);

      console.log('✅ deploy trend telemetry suppression alert suppression trend test passed');
    } finally {
      await shutdown();
    }

    await db.ensureDeployRunEventsTable();
    await db.postgres.query(`DELETE FROM deploy_run_events WHERE payload->>'test_suite' = 'deploy_trend_telemetry_suppression_alert_suppression_trend'`);
...
run().catch((err) => {
  console.error('❌ deploy trend telemetry suppression alert suppression trend test failed:', err.message);
  process.exit(1);
});
